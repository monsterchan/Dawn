...
# resultJson = {'person':0, 'bicycle':0, 'car':0, 'motorcycle':0, 'airplane':0, 'bus':0, 'train':0, 'truck':0, 'boat':0, 'traffic light':0,
...
def detect(images,save_img=False):
    weights = 'weights/yolov5s.pt'
    out = './inference/outImages'
    source = images
    view_img = False
    imgsz = 640
    save_txt = True
    ...
    if os.path.exists(out):
        shutil.rmtree(out)  # delete outImages folder
    os.makedirs(out)  # make new outImages folder
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            resultLable = s[8:]
            # Stream results
    return resultImage,resultLable
