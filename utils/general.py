...
def compute_loss(p, targets, model):  # predictions, targets, model
    ...
    s = 3 / np  # outImages count scaling
    lbox *= h['giou'] * s
...
def coco_single_class_labels(path='../coco/labels/train2014/', label_class=43):
    ...
    if os.path.exists('new/'):
        shutil.rmtree('new/')  # delete outImages folder
    os.makedirs('new/')  # make new outImages folder
...
def output_to_target(output, width, height):
    # Convert model outImages to target format [batch_id, class_id, x, y, w, h, conf]
